...
exports.bookingsCleanup = onSchedule('every day 00:00', async (event) => {
  ...
  const querySnapshot = await bookingsRef.where('day', '==', previusDate).get()
  ...
  console.log('batch length', batch.length ? batch.length : 'batch length is not a property')

  // Commit the batch
  await batch.commit()
  console.log('deleted bookings', doc.id)

  /*===== Set the reserved number of classes back to 0 on each class on previus day =====*/
